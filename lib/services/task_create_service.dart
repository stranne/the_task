...
When creating the next task, use all provided data such as previous tasks, the latest life goals, and any other meta-data (e.g., current time of day, weather) to customize a task that is a good next step. Select one life goal or combine multiple goals to create a task. Ensure the task is contextually relevant and specific to the user’s current situation. Make sure each new task is unique, and can't be seen as similar, or an alternative version, of a previous task, which are provided in the previous task list.
